 /******************************************************************************
  *
  *  File Name: 8139too-4.4-ethercat.c, 2019/03/09 19:17:16
  *
  *  Copyright (C) 2019, ICS, Harbin Institute of Technology
  *  
  *  Author: Xiaoke DENG
  *
...
#define DRV_NAME	"ec_8139too" ///add 128
...
#include <asm/irq.h>    ///add begin 151

#include "../globals.h"
#include "ecdev.h"

#define RTL8139_DRIVER_NAME  \
                    DRV_NAME "EtherCAT-capable Fast Ethernet driver "  \
...
MODULE_AUTHOR ("Xiaoke DENG <Xiaoke_deng@outlook.com>");
...
//		   rtl_chip_info[tp->chipset].name);
 	...
 	pr_info("%s: %s at 0x%p, %pM, IRQ %d\n",
 		dev->name, ///here may be wrong....
 		board_info[ent->driver_data].name,
 		ioaddr, ///here may be wrong.... base_addr becomes ioaddr
 		dev->dev_addr,
 		pdev->irq);
...
///add begin 2031
 	...
 	while ((tp->ecdev || netif_running(dev)) && received < budget &&
...
///add begin 2076
			...
			if (netif_msg_intr(tp)) {
 				pr_debug("%s: fifo copy in progress.", dev->name);
			}
///add end 2076
...
///on 3.2 is skb = netdev_alloc_skb_ip_align(dev, pkt_size);
            if (likely(skb)) {
#if RX_BUF_IDX == 3
                ...
                u64_stats_update_begin(&tp->rx_stats.syncp); /// no this on 3.2
                tp->rx_stats.packets++;
                tp->rx_stats.bytes += pkt_size;
                u64_stats_update_end(&tp->rx_stats.syncp); /// no this on 3.2
